...
export default function SwipeScreen({ route, navigation }: Props) {
  ...
  function loadCards(
    ...
    // 1) compute distance for every school
    ...
    let filtered: CollegeCard[];

    // 2) If a state is chosen, ignore radius entirely
    if (state) {
      filtered = mapped.filter(c => locationMatchesState(c.state, state));
    }
    // 3) Else if a region is chosen, ignore radius entirely
    else if (region) {
    ...
    // 4) Otherwise, fall back to your radius filter
    else {
      filtered = mapped.filter(c => c.distance <= radius);
    }

    // 5) Sort by nearest first in all cases
    ...
    Animated.timing(position, {
      ...
      if (nxt >= cards.length) {
        if (!extended && userCoords) {
          // once the deck is exhausted, show absolutely everything
